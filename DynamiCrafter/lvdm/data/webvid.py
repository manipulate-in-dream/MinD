...
class WebVid(Dataset):
    ...
    def __getitem__(self, index):
        # print(f"Fetching item {index} from dataset")
        if self.random_fs:
        ...
        while True:
            ...
            # 随机选择起始帧索引
            start_idx = random.randint(0, frame_num - 1)  # 随机生成起始索引

            # 计算帧索引
            frame_indices = [start_idx + frame_stride * i for i in range(self.video_length)]

            # 如果索引超出帧数范围（静态 case），补充最后一帧
            frame_indices = [
                idx if idx < frame_num else frame_num - 1  # 超出范围的索引用最后一帧替代
...
if __name__== "__main__":
    meta_path = "/scratch/vgenfmod/video_data/open_x/embodied_merged.csv" ## path to the meta file
    data_dir = "/scratch/vgenfmod/eloisa/DynamiCrafter/lvdm/data/Calvin_Robotics" ## path to the data directory
    save_dir = "/scratch/vgenfmod/eloisa/DynamiCrafter/finetuning" ## path to the save directory
    ...
    for i, batch in tqdm(enumerate(dataloader), desc="Data Batch"):
        ...
        # print(f"Video path: {batch['path']}")
        # print(f"FPS: {batch['fps']}")
        # print(f"Frame stride: {batch['frame_stride']}")
