...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        # self.final_layer = SepFinalLayer(hidden_size, self.out_channels)
        self.final_layer = FinalLayer(hidden_size, self.out_channels)
        # self.gripper_layer = FinalLayer(hidden_size, 1)
    ...
    def forward(self, x, t, z):
        ...
        # return x[:, 1:, :] 
        output = self.final_layer(x)                             # (N, T+1, out_channels)
        # 对最后一个维度（夹爪）应用 Sigmoid
        # action_output = output[..., :-1]  # (N, T+1, action_channels)
        # gripper_output = self.gripper_layer(x)  # (N, T+1), 单独对夹爪维度应用 Sigmoid
        # gripper_output = torch.tanh(self.gripper_layer(x))  # (N, T+1), 单独对夹爪维度应用 Sigmoid
        # 合并动作和夹爪输出
        # output = torch.cat([action_output, gripper_output], dim=-1)
        return output[:, 1:, :]     # (N, T, C)
...
class SepFinalLayer(nn.Module):
    def __init__(self, hidden_size, out_channels):
        """
        FinalLayer with separate outputs for action and gripper.
        Args:
            hidden_size: Size of the input features.
            out_channels: Total number of output channels (action + gripper).
        """
        super().__init__()
        self.hidden_size = hidden_size
        self.action_channels = out_channels - 1  # 动作维度
        self.gripper_channels = 1  # 夹爪维度

        # 动作输出层
        self.action_layer = nn.Linear(hidden_size, self.action_channels)
        # 夹爪输出层
        self.gripper_layer = nn.Linear(hidden_size, self.gripper_channels)

    def forward(self, x):
        """
        Forward pass for the final layer.
        Args:
            x: Input tensor of shape (N, T+1, hidden_size).
        Returns:
            Output tensor of shape (N, T+1, out_channels).
        """
        action_output = self.action_layer(x)  # (N, T+1, action_channels)
        gripper_output = self.gripper_layer(x)  # (N, T+1, gripper_channels)

        # 合并动作和夹爪输出
        return torch.cat([action_output, gripper_output], dim=-1)  # (N, T+1, out_channels)