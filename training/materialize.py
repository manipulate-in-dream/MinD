...
from training.strategies import VgmFSDPStrategy, VgmACTTrainingStrategy
VGM_TRAIN_STRATEGIES = {
    "fsdp-shard-grad-op": {"cls": VgmFSDPStrategy, "kwargs": {"sharding_strategy": "shard-grad-op"}},
    "fsdp-full-shard": {"cls": VgmFSDPStrategy, "kwargs": {"sharding_strategy": "full-shard"}},
}
def get_vgm_train_strategy(
    train_strategy: str,
    vlm: Union[PrismaticVLM, CogACT],
    device_id: int,
    stage: str,
    ...
    mixed_precision_dtype: torch.dtype = torch.bfloat16,
    worker_init_fn: Optional[Callable[[int], None]] = None,
) -> VgmACTTrainingStrategy:
    if train_strategy in VGM_TRAIN_STRATEGIES:
        strategy_cfg = VGM_TRAIN_STRATEGIES[train_strategy]
        strategy = strategy_cfg["cls"](
            vlm=vlm,
            device_id=device_id,
            stage=stage,
